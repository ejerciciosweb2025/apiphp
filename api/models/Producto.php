<?php
    ...
    class Producto{
        ...
        public function leer_individual(){
            //Crear query
            $query = 'SELECT c.nombre as categoria_nombre, p.id, p.categoria_id, p.titulo, p.texto, p.fecha_creacion FROM ' . $this->table . ' p LEFT JOIN categorias c ON p.categoria_id = c.id WHERE p.id = ? LIMIT 0,1';
